---
title: "Work Task Sixteen: Implement a DataSHIELD compatible compositions::acomp"
authors: "Xavier Escribà Montagut & Stuart Wheater"
format:
  html:
...
The work task involves implementing a DataSHIELD compatible version of the compositions::acomp function from the compositions R package.


## What does compositions::acomp do?

The `compositions::acomp` function creates objects for analyzing compositional data. It represents closed compositions where parts sum to a specified total (typically 1) and handles missing data including values below detection limits and structural zeros. The acomp class enables proper analysis using specific operations. ([Documentation](https://www.rdocumentation.org/packages/compositions/versions/2.0-8/topics/acomp))

## How is compositions::acomp implemented as a DataSHIELD compatible function?

The compositions::acomp function is implemented as a DataSHIELD assign function (acompDS) since it needs to operate on data that resides on the analysis server. It takes a table as input, processes it on the server-side to create a new `acomp` object.

## Solution

A 2-stage assign DataSHIELD function has been developed and placed into dsCoda. This function acts as a wrapper to the target function compositions::acomp.
...
library('dsCodaClient') # Make sure this is task-17 branch `devtools::install_github("datashield/dsCodaClient", ref = "task-16")`
```
...
We will pass the subseted data frame to the `acomp` function.

```{r acomp, results='hide', cache=FALSE}
ds.acomp(
  X = "D4",
  objectname = "D5"
)
...
This task 16 analysis demonstrates the complete DataSHIELD workflow to use the `acomp` function, we included:
...
4. **acomp**: Create a new `acomp` object.
...
The `ds.acomp` returns a new `acomp` object.
